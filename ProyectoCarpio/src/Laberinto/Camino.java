...
public class Camino {
    ...
    /*
    private Genotipo generarNuevoGenotipo(Hijo h, int anterior, Genotipo genotipo) {
        if (h == null || !h.isExplorado()) {
            return null;
        }
        int next = Genetico.reemplazo(h, genotipo);
        switch (next) {
            case 0:
        }
        ...
        if (!(Camino.NORTE == anterior)) {
            generarNuevoGenotipo(h.getNorte(), Camino.SUR);
        }
        if (!(Camino.SUR == anterior)) {
            generarNuevoGenotipo(h.getSur(), Camino.NORTE);
        }
        if (!(Camino.ESTE == anterior)) {
            generarNuevoGenotipo(h.getEste(), Camino.OESTE);
        }
        if (!(Camino.OESTE == anterior)) {
            generarNuevoGenotipo(h.getOeste(), Camino.ESTE);
        }
